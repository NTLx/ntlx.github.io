...
- [run-with-cdn-fallback.sh](file://.agents/skills/wechat-article-write/scripts/run-with-cdn-fallback.sh)
...
## 更新摘要
**变更内容**
- 更新微信文章写作管线为15步完整流程，包含新增的Step 2.5和Step 4.6
- 新增现代化状态管理系统，使用state-lib.mjs提供统一的状态写入机制
- 引入CDN集成和降级处理，通过run-with-cdn-fallback.sh实现智能CDN直连和本地降级
- 明确博客优先发布策略，先发布博客版本再发布微信草稿
- 增强错误处理和失败回退机制，每个步骤都有详细的容错处理
- 优化并行执行策略，封面、插图、信息图的并行处理更加完善
...
**章节来源**
- [.agents/skills/wechat-article-write/SKILL.md:1-1458](file://.agents/skills/wechat-article-write/SKILL.md#L1-L1458)
- [.agents/skills/baoyu-post-to-wechat/SKILL.md:1-268](file://.agents/skills/baoyu-post-to-wechat/SKILL.md#L1-L268)
- [.agents/skills/github-image-hosting/SKILL.md:1-107](file://.agents/skills/github-image-hosting/SKILL.md#L1-L107)

## 核心组件
- **wechat-article-write**
  - **15步完整流程**：资料收集 → 文章创作 → 质量门控 → 分类确认 → 封面/信息图/插图并行 → 统一格式检测 → 图床前置上传 → 占位符替换 → 去 AI 痕迹 → Markdown 格式化 → HTML 转换（CDN降级） → 博客发布轨（先发布） → 等待Pages部署（可选） → 微信发布轨（后发布）
  - **状态管理系统**：state.mjs（命令行工具）+ state-lib.mjs（库函数），提供统一的状态读写和断点续跑能力
  - **CDN集成**：run-with-cdn-fallback.sh 实现CDN直连超时自动降级到本地路径的智能处理
  - **关键门控**：质量门控、引用验证、格式统一检测、CDN降级验证
...
**章节来源**
- [.agents/skills/wechat-article-write/SKILL.md:10-1458](file://.agents/skills/wechat-article-write/SKILL.md#L10-L1458)
...
- [.agents/skills/wechat-article-write/scripts/run-with-cdn-fallback.sh:1-159](file://.agents/skills/wechat-article-write/scripts/run-with-cdn-fallback.sh#L1-L159)
...
- [.agents/skills/wechat-article-write/scripts/publish-wechat.mjs:1-116](file://.agents/skills/wechat-article-write/scripts/publish-wechat.mjs#L1-L116)
- [.agents/skills/wechat-article-write/scripts/step46-upload-cdn.mjs:1-83](file://.agents/skills/wechat-article-write/scripts/step46-upload-cdn.mjs#L1-L83)
- [.agents/skills/wechat-article-write/scripts/validate-pipeline.sh:1-154](file://.agents/skills/wechat-article-write/scripts/validate-pipeline.sh#L1-L154)
...
- **编排层**：wechat-article-write 负责15步完整流水线与门控，包含状态管理和CDN降级处理
...
W->>W : CDN降级处理run-with-cdn-fallback.sh
W->>P : 发布草稿API/浏览器
alt API 模式
P->>WX : 获取访问令牌
P->>WX : 上传正文图片/封面
...
- [.agents/skills/wechat-article-write/scripts/run-with-cdn-fallback.sh:17-25](file://.agents/skills/wechat-article-write/scripts/run-with-cdn-fallback.sh#L17-L25)
...
**更新** 微信文章写作管线现已升级为15步完整流程，包含新增的分类确认和图床前置上传步骤。
...
- **Step 5**：占位符→CDN URL（硬门控）→ 生成article.md和article-local.md双产物
- **Step 6**：去AI痕迹（硬门控）→ humanizer-zh检测与修正
- **Step 7**：Markdown格式化 → baoyu-format-markdown排版优化
- **Step 8**：HTML转换（CDN降级）→ run-with-cdn-fallback.sh智能处理
...
#### CDN集成与降级处理
- **run-with-cdn-fallback.sh**：实现CDN直连超时自动降级到本地路径
- **双产物机制**：同时生成article.md（CDN版）和article-local.md（本地降级版）
- **智能回写**：本地降级成功后自动将article.html中的本地路径回写为CDN URL
...
U --> V["Step 6 去AI痕迹"]
V --> W["Step 7 Markdown格式化"]
W --> X["Step 8 HTML转换CDN降级"]
X --> Y["Step 9 博客发布轨"]
Y --> Z["Step 9.5 等待Pages部署"]
Z --> AA["Step 10 微信发布轨"]
AA --> BB["结束"]
```
...
- **wechat-article-write 脚本依赖**
  ...
  - run-with-cdn-fallback.sh：CDN直连与降级处理
...
A --> C["run-with-cdn-fallback.sh"]
A --> D["validate-pipeline.sh"]
A --> E["publish-blog.mjs"]
A --> F["publish-wechat.mjs"]
A --> G["step46-upload-cdn.mjs"]
H["wechat-api.ts"] --> I["@jsquash/webp"]
H --> J["baoyu-md"]
H --> K["jimp"]
L["upload.ts"] --> M["gh CLI"]
```
...
- [.agents/skills/wechat-article-write/scripts/run-with-cdn-fallback.sh:109-116](file://.agents/skills/wechat-article-write/scripts/run-with-cdn-fallback.sh#L109-L116)
...
- **CDN降级智能处理**
  - run-with-cdn-fallback.sh 自动检测CDN超时并降级到本地路径
  - 双产物机制确保CDN失败时仍能继续执行
- **状态管理优化**
...
- [.agents/skills/wechat-article-write/scripts/run-with-cdn-fallback.sh:121-156](file://.agents/skills/wechat-article-write/scripts/run-with-cdn-fallback.sh#L121-L156)
...
- **15步流程特定问题**
  ...
  - Step 5 CDN降级验证失败：检查image-map.json完整性，确认占位符与图片编号对齐
...
- **CDN降级问题**
  - CDN超时：等待30秒CDN缓存传播，或使用本地降级模式
  - 本地降级路径未回写：检查run-with-cdn-fallback.sh的--html-rewrite参数
...
## 结论
发布自动化技能通过15步完整流程和现代化的状态管理、CDN集成、博客优先发布策略，实现了从资料收集到文章发布的全链路自动化。wechat-article-write 提供了强大的编排能力，包含智能CDN降级、并行执行优化、统一状态管理等现代化特性；baoyu-post-to-wechat 提供可靠的 API 与浏览器双通道发布；github-image-hosting 则保障图片资源的稳定分发。配合完善的多账号与权限管理、错误处理与性能优化策略，整体方案具备良好的可维护性与扩展性。
