...
- [baoyu-imagine/references/config/preferences-schema.md](file://.agents/skills/baoyu-imagine/references/config/preferences-schema.md)
...
- [baoyu-translate/references/config/extend-schema.md](file://.agents/skills/baoyu-translate/references/config/extend-schema.md)
</cite>
...
### 专用技能
- baoyu-translate
  - 三模式翻译（快速/正常/精细），支持术语表合并与长文分块。
  - 自动识别语言对与风格，输出结构化中间文件以便审阅与润色。
- 其他专用技能（如 ljg-*、web-access 等）
  - 面向特定领域或工具链的补充能力，遵循相同的 EXTEND.md 与 SKILL.md 设计。
...
## 结论
NTLx's Blog 的 AI 技能系统通过“可插拔技能包 + 偏好驱动 + 可复现工作流”的设计，实现了从内容创作到发布的全链路自动化。EXTEND.md 提供统一的配置入口，skills-lock.json 确保版本与来源可控，而标准化的工作流与提示词文件则保障了结果的一致性与可追溯性。建议在团队内推广该体系，持续完善参考文档与示例，提升协作效率与交付质量。
...
- 使用场景与集成建议
  ...
  - 翻译与本地化：baoyu-translate，配合术语表与风格偏好。
- 最佳实践
