...
- [src/content/docs/guides/authoring-guide.md](file://src/content/docs/guides/authoring-guide.md)
</cite>
...
- 补充文档版本管理的最佳实践说明
- 更新技术栈版本：框架升级到 Astro v6 和 Starlight v0.39
- 更新技能数量统计：从 37+ 减少到 28 个
- 新增技术博文编写规范的详细说明
...
NTLx's Blog 是一个基于 Astro Starlight v0.39 构建的个人知识库网站，旨在系统化记录与分享技术笔记与深度思考。项目采用双发布能力：既可以直接在 `src/content/docs/` 下创建 Markdown 文章，通过 GitHub Actions 自动部署到 GitHub Pages；也可以通过 wechat-article-write 技能，将 URL 或素材自动转化为微信公众号文章与博客文章，实现"一条命令，双端发布"。

**重要更新**：项目文档结构已重新组织，CLAUDE.md 文件已被 AGENTS.md 替代，同时保留了 GEMINI.md 作为备用文档。所有文档文件内容保持一致，确保向后兼容性和文档版本管理的灵活性。项目框架已升级到 Astro v6，技能生态系统从 37+ 个技能精简到 28 个核心技能。
...
- [.agents/skills/wechat-article-write/SKILL.md:1-200](file://.agents/skills/wechat-article-write/SKILL.md#L1-L200)
...
- [CLAUDE.md:1-87](file://CLAUDE.md#L1-L87)
- [GEMINI.md:1-87](file://GEMINI.md#L1-L87)
...
- **更新**：技术博文编写规范
  - 新增独立的 `src/content/docs/guides/authoring-guide.md` 作为技术博文编写规范
  - 详细涵盖联网工具使用、跨平台命令、Asides 组件、代码块与 Git 提交等编写约定
...
- [src/content/docs/guides/authoring-guide.md:1-97](file://src/content/docs/guides/authoring-guide.md#L1-L97)
...
**重要更新**：架构中新增文档版本管理层，支持多工具集成场景下的文档版本管理。框架已升级到 Astro v6，Starlight 版本为 v0.39。
...
D4["authoring-guide.md<br/>技术博文规范"]
end
C1 --> S1
C2 --> S1
C3 --> S1
...
- [skills-lock.json:1-241](file://skills-lock.json#L1-L241)
- [AGENTS.md:43-51](file://AGENTS.md#L43-L51)
- [src/content/docs/guides/authoring-guide.md:17-97](file://src/content/docs/guides/authoring-guide.md#L17-L97)
...
- 导航与侧边栏
  ...
  - **更新**：Sidebar autogenerate v0.39+ 要求 `autogenerate` 必须嵌套在 `items: [{ autogenerate: { ... } }]` 内
- SEO 与编辑链接
...
**新增**：项目新增独立的技术博文编写规范文档，详细规定了内容创作标准。

- 联网搜索与浏览器使用
  - 统一通过 web-access 技能完成联网操作
  - 禁止使用任何 MCP 搜索工具、WebFetch、WebParser 或其他网络工具
- 技术博文编写规范
  - 信息层次设计："是什么 → 为什么 → 怎么做 → 出问题怎么办"
  - 跨平台兼容性：同时提供 Linux/macOS/Windows 三个平台的命令示例
  - Starlight Asides 组件使用：tip、caution、note 三种类型
  - 代码块规范：添加语言标识符、注释说明、临时/永久配置方式
- 环境变量配置文档模式
  - 明确环境变量 > 配置文件的优先级
  - 提供多种配置方案满足不同技能水平用户
- 故障排除章节编写
  - 问题导向：直接列出用户可能遇到的具体症状
  - 提供选择：完全重置 vs 精确操作
  - 风险提示：在执行危险操作前使用 caution 明确告知后果
- 开发服务器使用
...
- 技术栈
  ...
  - 公众号发布：微信公众号 API
- 外部依赖
  - 技能链：baoyu-* 系列（12个）、ljg-* 系列（15个）、web-access、humanizer-zh 等
...
- **新增**：文档版本管理依赖
  ...
  - 多工具集成场景下的文档选择策略
- **更新**：技能数量统计
  - 项目技能从 37+ 个精简到 28 个核心技能
  - baoyu 系列：12 个技能（baoyu-article-illustrator、baoyu-comic、baoyu-compress-image、baoyu-cover-image、baoyu-diagram、baoyu-format-markdown、baoyu-image-cards、baoyu-imagine、baoyu-infographic、baoyu-markdown-to-html、baoyu-post-to-wechat、baoyu-slide-deck、baoyu-youtube-transcript）
  - ljg 系列：15 个技能（ljg-invest、ljg-paper、ljg-paper-flow、ljg-paper-river、ljg-plain、ljg-present、ljg-qa、ljg-rank、ljg-relationship、ljg-think、ljg-writes、ljg-card、ljg-learn、ljg-read、ljg-roundtable、ljg-skill-map、ljg-travel、ljg-word、ljg-word-flow）
  - 其他技能：1 个（web-access）

```mermaid
graph TB
A["package.json 依赖"] --> B["astro@^6.3.1"]
A --> C["@astrojs/starlight@^0.39.2"]
A --> D["sharp@^0.34.2"]
E["astro.config.mjs"] --> F["Starlight v0.39 集成"]
...
- **更新**：框架性能提升
  - Astro v6 相比 v5 在构建性能和运行时性能上有显著提升
  - Starlight v0.39 优化了内容加载和导航性能
...
- **新增**：技术博文编写规范问题
  - 现象：文章不符合技术博文编写规范
  - 处理：参考 `src/content/docs/guides/authoring-guide.md` 的详细规范进行修改
- **更新**：Sidebar autogenerate v0.39+ 配置错误
  - 现象：侧边栏无法正确生成
  - 处理：确保 `autogenerate` 嵌套在 `items: [{ autogenerate: { ... } }]` 内
...
NTLx's Blog 通过 Astro Starlight v0.39 提供优秀的文档体验，结合 wechat-article-write 技能链实现了"写作—发布—传播"的高效闭环。项目在内容组织、技能编排、版本管理与基础设施方面形成了清晰的架构，既能满足初学者快速上手，也能为有经验的开发者提供深度定制与扩展空间。

**重要更新**：项目文档结构的重新组织体现了对多工具集成场景的支持，通过 AGENTS.md、CLAUDE.md、GEMINI.md 的多版本文档管理，确保了向后兼容性和工具集成的灵活性。框架升级到 Astro v6 和 Starlight v0.39 带来了性能提升，技能生态系统从 37+ 个精简到 28 个核心技能，提高了系统的稳定性和可维护性。新增的技术博文编写规范为内容创作提供了标准化指导。

建议在使用过程中关注并行策略、格式检测与引用校验等关键环节，同时注意文档版本的一致性管理，以确保流水线稳定可靠。技术博文编写规范的实施将显著提升内容质量和用户体验。
...
- **新增**：技术博文编写规范
  - 独立的 `src/content/docs/guides/authoring-guide.md` 提供详细的内容创作指导
  - 涵盖联网工具使用、跨平台命令、Asides 组件、代码块与 Git 提交等编写约定
- **更新**：技能生态系统
  - 核心技能数量：28 个（baoyu 系列 12 个 + ljg 系列 15 个 + web-access）
  - 技能精简：从 37+ 个技能优化到 28 个核心技能，提高系统稳定性
  - 技能锁定：通过 skills-lock.json 确保技能版本可复现
...
- [src/content/docs/guides/authoring-guide.md:87-97](file://src/content/docs/guides/authoring-guide.md#L87-L97)