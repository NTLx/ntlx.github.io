...
将以下脚本保存为 `install_lsp.sh`，自动检测已安装的包管理器并安装对应 LSP：
...
echo ""
echo "=========================================="
echo -e " 安装完成: ${GREEN}${installed}${NC} 种  跳过: ${YELLOW}${skipped}${NC} 种"
...
将以下脚本保存为 `update_lsp.sh`，自动检测并更新所有已安装的 LSP：
...
echo ""
echo "=========================================="
echo -e " 更新完成: ${GREEN}${updated}${NC} 组  跳过: ${YELLOW}${skipped}${NC} 组"
echo " 请重启 Claude Code/OpenCode 以生效"
echo "=========================================="
```
...
```bash
# 赋予执行权限
chmod +x install_lsp.sh update_lsp.sh
...
脚本会自动检测系统中已安装的包管理器（bun/npm/uv/rustup/cpanm），只安装/更新对应工具链的 LSP，未检测到的会跳过并给出安装提示。
:::
