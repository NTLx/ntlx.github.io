...
| step4 报 cover missing、双 root cover 或 coverImage 未规范化 | 在 humanizer 前运行 `pre-humanizer-normalize.mjs <date-slug>`，它会处理 `imgs/00-cover.*` / `imgs/cover.*` 归位、MIME/扩展名和 `coverImage`；root `cover.png` / `cover.jpg` 必须恰好一个。receipt 产生后 Step 4 不再自动修改 `draft.md`。若 cover 是之后由 renderer 生成的，必须直接写到 post 根目录并匹配 frozen 的 `coverImage` |
