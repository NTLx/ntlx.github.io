...
Gate 第一次失败，返回 `RETRY_REQUIRED`，将 bounded failure class/counts/samples 传回 gzh-design，
从冻结的 `article-wechat-source.md` 重新生成一次。若 fresh retry 后仍为同一 failure class，返回
`BLOCKED`；不得第三次自动 retry 或轮换主题（theme roulette）。
失败将实际诊断传回 gzh-design，从冻结的 `article-wechat-source.md` 重新生成；
