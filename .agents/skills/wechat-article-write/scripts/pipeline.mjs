...
1. native delegate github-image-hosting → image-map.json
2. run step5-build.mjs ${slug} --prepare-only
3. native delegate gzh-design → article-wechat.html (including its validator and preview)
4. run step5-build.mjs ${slug} --finalize-only
`);
  process.exit(0);
}
...
1. run publish-blog.mjs ${slug}
2. run publish-wechat.mjs ${slug} --prepare-only
3. native delegate baoyu-post-to-wechat
4. run publish-wechat.mjs ${slug} --finalize-only after child success
`);
  else process.stdout.write(`Next:
1. run publish-wechat.mjs ${slug} --prepare-only
2. native delegate baoyu-post-to-wechat
3. run publish-wechat.mjs ${slug} --finalize-only after child success
`);
  process.exit(0);
}
