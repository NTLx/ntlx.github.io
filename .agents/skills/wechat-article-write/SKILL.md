...
**双轨分离原则**：博客轨只消费 Markdown + CDN URL（Astro Starlight 直接构建，不生成 HTML），微信轨只消费本地文件（article-wechat.html + imgs/ 本地路径，wechat-api.ts 直接读本地文件上传）。两条轨道零共享中间产物。

**脚本化原则**：本文档原本散落的 bash/python 校验、占位符替换、frontmatter 转换等逻辑全部封装到 `scripts/` 下的可复用脚本（`state.mjs` / `set-frontmatter.mjs` / `suggest-category.mjs` / `apply-image-map.mjs` / `validate-pipeline.sh` / `publish-blog.mjs` / `wait-pages-deployed.mjs` / `publish-wechat.mjs`）。`run-with-cdn-fallback.sh` 已废弃（微信轨全程本地文件，不存在 CDN 降级场景）。Agent 只负责"按 Step 调用脚本 + 解读退出码 + 写入 `.pipeline-state.json`"，**严禁内联超过 5 行的 bash/python 代码**。
...
| 调用方式 | 适用技能 | 执行方式 |
|---------|---------|---------|
| **Skill 工具调用** | baoyu-cover-image、ljg-writes、humanizer-zh、baoyu-article-illustrator、baoyu-infographic、web-access（联网入口，**唯一**） | 通过 Skill 工具加载 SKILL.md，agent 按其流程执行 |
| **脚本执行** | github-image-hosting、baoyu-imagine、baoyu-format-markdown、baoyu-markdown-to-html、baoyu-post-to-wechat | 查找 `{skillDir}/scripts/` 目录下的脚本文件 |

> **联网铁律**：本流水线中**所有联网/抓取/搜索/浏览**操作，**只能**通过 **web-access** 技能完成。**禁止**使用任何 MCP 联网工具、WebFetch、WebParser、curl/wget 抓取页面，**也不得**回退到任何已弃用技能（包括但不限于已删除的 `baoyu-url-to-markdown`、`baoyu-fetch` 等）。如果 web-access 不可用，**直接询问用户手动提供内容**，不要寻找替代联网工具。
...
> **发布顺序：先博客后微信**。博客 commit/push 触发 GitHub Pages 部署在前，微信草稿在后。原因：①博客是文章权威 URL（`sourceUrl`）来源；②博客发布只更新仓库文件、可控且可秒回滚；③微信草稿写入后需要人工审核再群发。sourceUrl 在 Step 2 创作时已预先填入博客 URL（`https://ntlx.github.io/articles/{slug}/`），Step 10 直接使用该 URL 发布——读者可能在 push 后 1-3 分钟内看到 404，但 Pages 部署通常很快。**如需确保零 404，可手动运行 Step 9.5（wait-pages-deployed.mjs）等待部署完成后再执行 Step 10。**
...
| 4.6 | 图床上传（前置 CDN） | github-image-hosting | image-map.json | ⛔ |
| 5 | 占位符 → CDN URL | apply-image-map.mjs | article.md（博客 CDN 版）+ article-local.md（微信本地路径版） | ⛔ |
| 6 | 去 AI 痕迹 | humanizer-zh（**必须执行，不可跳过**） | article.md（优化后） | ⛔ |
| 7 | Markdown 格式化 | baoyu-format-markdown | article.md（排版后） | |
| 8 | HTML 转换（微信轨专用）⛔ 必须执行 | baoyu-markdown-to-html（从 article-local.md 生成） | article-wechat.html | ⛔ |
| **9** | **博客发布轨**（先发布） | publish-blog.mjs：frontmatter 转换 + astro sync + git push（仅消费 article.md Markdown，不涉及 HTML） | src/content/docs/articles/{slug}.md → 触发 Pages 部署 | ⛔ |
| 9.5 | 等待 Pages 部署就绪（**可选**） | wait-pages-deployed.mjs：轮询 `https://ntlx.github.io/articles/{slug}/` 返回 200 | 部署确认 | |
| **10** | **发布到公众号草稿**（后发布） | publish-wechat.mjs（使用 article-wechat.html + 本地图片路径，零 CDN 依赖） | 公众号草稿 | ⛔ |

> **状态写入全内聚**：流水线所有 Step 均通过脚本内置状态写入，agent 只需调用对应脚本。**不再需要手动调用 `state.mjs set`**。断点续跑用 `bun run scripts/state.mjs next <date-slug>`。
...
**并行完成后的强制序列**（不得跳过）：
1. Step 3.4.2 统一格式检测（file 命令检测所有图片，修正 .png → .jpg）
2. Step 4-4.5 插图+信息图状态写入（见下方脚本）
...
## date-slug 与 blog-slug 生成规则

**两条 slug，两条规则**：

### date-slug（流水线工作目录名）

从文章标题生成安全的目录名，用于 `posts/` 下存放中间产物：
...
- date-slug：`2026-04-30-深入理解ReactHooks的设计哲学`

> date-slug **允许中文**，因为它只是本地目录名，不进入 URL。

### blog-slug（博客文件名，必须纯 ASCII）

博客文件名是 URL 的一部分（`ntlx.github.io/articles/<slug>/`），必须为小写 ASCII kebab-case。**CLAUDE.md 硬规则禁止中文文件名**。

生成规则：

1. 在 Step 2.5 分类确认时，**同时生成一个 ASCII slug**（由 agent 根据文章主题翻译/提炼）
2. 格式：`{2-4个英文关键词-kebab-case}`，简短、可读、与主题相关
3. 必须满足：`^[a-z][a-z0-9-]*[a-z0-9]$`（纯小写 ASCII，无连续 hyphen）
4. 如果 agent 无法自动生成满意的英文 slug，询问用户指定

**示例**：
- 文章标题：「频谱分析检测假无损音乐」→ blog-slug：`fake-lossless-spectrum-analysis`
- 文章标题：「Agentic Workflow 烧掉的钱去哪了？」→ blog-slug：`agentic-workflow-costs`
- 文章标题：「深入理解 React Hooks 的设计哲学」→ blog-slug：`react-hooks-design-philosophy`

**publish-blog.mjs 的 --blog-slug 参数**：脚本新增 `--blog-slug <ascii-slug>` 参数，显式传入 ASCII slug。如果未传此参数，脚本从 date-slug 自动去掉日期前缀后验证是否纯 ASCII——如果不是，报错退出（exit 2），要求 agent 传入 `--blog-slug`。
...
| 技能 | 检查路径 | 检查条件 |
|-----|---------|---------|
| web-access | `{skillDir}/scripts/` 存在 | 且 `scripts/check-deps.mjs` 存在；CDP 浏览器可达 |
...
**关键**：脚本类型检测仅用于本流水线现存的脚本型技能（github-image-hosting / baoyu-format-markdown / baoyu-markdown-to-html / baoyu-post-to-wechat / baoyu-imagine）。所有联网/抓取需求一律走 web-access Skill 调用，不要再去查找任何 baoyu-fetch / baoyu-url-to-markdown 类的脚本入口（已弃用并从仓库移除）。
...
**脚本内置**：使用 `step0-check-deps.mjs` 自动完成依赖检查与状态写入，不再需要手动调用 `state.mjs set`。

```bash
bun run .agents/skills/wechat-article-write/scripts/step0-check-deps.mjs \
  <date-slug> [--skills web-access,github-image-hosting,baoyu-format-markdown,baoyu-markdown-to-html,baoyu-post-to-wechat,baoyu-infographic]
```
...
### 1.2 URL 内容获取（**唯一入口：web-access**）

> **铁律**：URL 内容获取**只能**通过 web-access Skill 完成。**不得**使用 baoyu-url-to-markdown / baoyu-fetch / WebFetch / WebParser / curl / wget 或任何 MCP 联网工具。已弃用技能（如 baoyu-url-to-markdown）已从仓库移除，**不存在"后备方案"**——遇到 web-access 失败时直接降级到"用户手动提供"，不要寻找其他抓取工具。

**唯一执行路径**：

1. 通过 Skill 工具调用 **web-access**，传入目标 URL
2. web-access 内部使用 CDP 直连用户日常 Chrome：天然携带登录态，可处理动态渲染、反爬拦截
3. 失败处理（按场景分）：
   - 页面需要登录且当前未登录 → **暂停**，告知用户："页面需要登录，请在 Chrome 中登录 {网站名}，登录完成后告诉我继续。"用户登录后**直接刷新页面继续**，无需重启 web-access
   - CDP 不可用 / Chrome 未启动调试端口 → 报告环境问题，请用户启动 Chrome 调试模式后重试
   - 抓取超时、页面无内容、反爬硬阻断 → **直接进入第三级（用户手动提供）**，**不得**寻找替代抓取工具

**第三级：用户手动提供**

- 报告失败的 URL 与失败原因（登录/反爬/超时/其他），询问用户是否能手动提供内容
- 用户提供了 → 使用
...
bun run .agents/skills/wechat-article-write/scripts/step1-materials-done.mjs \
  <date-slug> [--sources <成功数> --failed <失败数>]
```

**为什么必须写入**：Step 1 是资料收集的起点，断点续跑依赖此状态判断是否需要重新收集资料。跳过状态写入会导致 `state.mjs next` 误报 Step 1 未执行。
...
ljg-writes 内部硬约束为 1000-1500 字且「一篇只说一个点」。**本流水线的字数下限统一为 2500 字**（公众号长文要求），高于 ljg-writes 默认上限 1500。因此 wechat-article-write 作为调度层**默认**就需要扩展模式传参覆盖 ljg-writes 的字数约束，**不存在"默认模式 1000-1500 字"这种降级选项**。是否进一步加大目标字数（3500+）由材料丰富度判断。

**默认目标**：2500-3500 字（标准扩展模式，所有文章必须达到）。

**进一步加大目标字数（3500-5000 字）的条件**（满足任一即可）：
- 原始材料超过 5000 字
- 材料涵盖 5 个以上独立论点/主题
- 用户明确要求超长文或多章节深度展开
...
```
扩展模式：材料丰富，目标 2500-3500 字（如材料尤其丰富则 3500-5000 字）。**字数下限 2500 是硬性门控，低于此值流水线 Step 2.4.1 会拒绝放行**。允许分章节深入展开多个论点。每个论点用「切」方法独立深挖，但共享同一个核心观点。覆盖 ljg-writes 的 1000-1500 字约束和「一篇只说一个点」限制——不是多个点浅层罗列，而是多个论点围绕同一核心各自切到底。
...
**关于"默认模式"的澄清**：本流水线**没有"默认模式"**。任何博客/公众号文章都需要 ≥ 2500 字。如果材料过于稀薄无法支撑 2500 字，应回到 Step 1 补充材料，**不要**降低字数门控。

### 2.3 内容规范

**字数**（**强制**，多点加固）：
- **下限：2500 字**（流水线硬门控，validate-pipeline.sh draft 阶段会拒绝放行）
- 标准目标：2500-3500 字
- 超长目标：3500-5000 字（材料尤其丰富时）
- 字数不是目标而是底线——少于 2500 说明没挖够，必须回去补；超过 5000 检查是否有水分
...
- 引用区块不纳入字数统计
- **纯文本 URL 必须保留**：`> {原文URL}` 这一行不得省略。微信对外部超链接（`<a href>`）有管控拦截，但纯文本 URL 不受限制——读者可以手动复制到浏览器打开。这是绕过微信外链管控的唯一可靠方式，也是对原文作者的基本引用礼貌
...
| 检查项 | 标准 | 不通过时的处理 |
|--------|------|---------------|
| 字数 | **≥ 2500 字（硬性下限，无降级）** | 回到 ljg-writes 补充内容，或手动扩展关键段落；**不得降低门控** |
| 文末互动 | 正文末尾必须有 `*...*` 格式的互动问题 | 手动补写互动问题 |
| 原文参考 | 必须有 `## 原文参考` 区块，包含来源 URL（纯文本形式，不得省略） | 手动补写原文参考及纯文本 URL |
...
>
> 同时请确认 blog-slug（博客文件名，纯 ASCII kebab-case）：推荐 **fake-lossless-spectrum-analysis**。可修改或自定义。
...
#### 2.5.3 写回 frontmatter（category + blogSlug）
...
bun run .agents/skills/wechat-article-write/scripts/set-frontmatter.mjs \
  posts/{date-slug}/draft.md set blogSlug {ascii-slug}
```
...
**脚本内置**：使用 `step25-set-category.mjs` 一键完成分类推荐 + frontmatter 写入 + 状态写入。

```bash
bun run .agents/skills/wechat-article-write/scripts/step25-set-category.mjs \
  posts/{date-slug}/draft.md --category {chosen} --blog-slug {ascii-slug}
```
...
sourceUrl: https://ntlx.github.io/articles/{blogSlug}  # 必须替换为 Step 2.5 确认的实际 blog-slug（纯 ASCII），不得使用字面量 {slug}
---
...
| `## 原文参考` 区块 | ✅（读后感类） | 包含作者、标题、来源 URL（纯文本行，不得省略） |
...
封面图几乎必定存在格式不匹配（Gemini 后端返回 JPEG 内容但保存为 `.png` 扩展名）。**不要在此处单独执行格式检测**——格式检测统一在 **Step 3.4.2**（封面+信息图+插图集中处理）执行，确保零遗漏。

本段仅说明封面格式检测的参数变更：检测后若输出 `JPEG image data`，需重命名 `cover.png` → `cover.jpg`，同时更新 draft.md frontmatter 中的 `coverImage` 字段，并更新 Step 9 发布命令中的 `--cover` 参数路径。
...
**脚本内置**：使用 `step3-cover-done.mjs`，自动检测封面格式（.png→.jpg）、更新 draft.md frontmatter、写状态。

```bash
bun run .agents/skills/wechat-article-write/scripts/step3-cover-done.mjs <date-slug>
```
...
```
封面图生成完成！
保存位置：posts/{date-slug}/cover.{png|jpg}
```
...
ref_count=$(grep -c '![](imgs/[0-9][0-9]-' posts/{date-slug}/draft.md)
```
...
**为什么必须在 4.5 内验证**：等到 Step 5（占位符 → CDN URL）或 Step 6（去 AI 痕迹）才发现缺少信息图，返工成本更高。在 Step 4.5 内部闭环验证，确保「生成 → 插入 → 验证」三步在同一上下文中完成。
...
**已移除**。博客是唯一消费 CDN URL 的渠道，Astro 构建不 fetch 图片（只是把 URL 写进 `<img src>` 标签），用户访问时 CDN 早已就绪。微信轨全程使用本地文件，不涉及 CDN。因此无需等待 CDN 传播。

### 4.6.4 状态写入

**脚本内置**：使用 `step46-upload-cdn.mjs` 一键完成上传 + 状态写入。

```bash
bun run .agents/skills/wechat-article-write/scripts/step46-upload-cdn.mjs \
  <date-slug> [--date YYYY-MM-DD]
```
...
**目的**：把 `draft.md` 中所有 `<!-- SLOT_IMG_NN_xxx -->` 占位符按 `image-map.json` 替换为 CDN URL，同时产出本地路径版作为微信轨的正式 Markdown 输入。
...
**前置门控**：Step 5（占位符 → CDN URL）必须已完成且验证通过。
...
**脚本内置**：使用 `step6-humanize-done.mjs` 自动验证 `article.md` 存在并写状态。

```bash
bun run .agents/skills/wechat-article-write/scripts/step6-humanize-done.mjs <date-slug>
```

## Step 7: Markdown 格式化

**前置门控**：Step 6（去 AI 痕迹）必须已完成。
...
Step 7 由 Skill 工具调用 baoyu-format-markdown 完成，agent 在调用后执行状态写入：

```bash
bun run scripts/state.mjs set <date-slug> 7 done
```

## Step 8: HTML 转换（微信轨专用）⛔ 必须执行

使用 `baoyu-markdown-to-html` 技能将 `article-local.md`（本地 imgs/ 路径版）转换为微信公众号兼容的 HTML，产物固定为 `article-wechat.html`。

**⚠️ 禁止跳过**：Step 8 是微信轨的关键步骤——Step 10（发布到公众号草稿）强依赖 `article-wechat.html`。跳过 Step 8 会导致 Step 10 的 pre-flight 校验失败（`article-wechat.html` 不存在）。agent 不可因任何理由跳过 Step 8，包括但不限于："博客轨不需要 HTML"、"Step 9 不是 Step 8 的前置"、"已经发布了博客"。每次流水线都必须执行。

**执行顺序**：Step 8 不是 Step 9（博客发布）的前置——博客轨不消费 HTML。但 Step 8 **是 Step 10（微信发布）的前置**。推荐执行顺序：Step 8 → Step 9 → Step 10。Step 8 和 Step 9 也可以并行执行（它们操作不同文件），但两者都必须在 Step 10 之前完成。

**双轨分离**：博客轨不消费 HTML——Astro Starlight 直接消费 `article.md`（Markdown + CDN URL），构建时自动转为 HTML。微信轨使用本地路径版 HTML，`wechat-api.ts` 的 `loadUploadAsset()` 直接通过 `fs.readFileSync` 读取 `imgs/` 下的图片文件上传到微信素材库。两条轨道零共享中间产物。
...
### 8.2 调用 HTML 转换（首选方式：一键脚本）

**推荐使用 `step8-html-convert.mjs`**，一键完成 HTML 转换 + 重命名 + 验证 + 状态写入，无需手动查找脚本路径或执行 `mv`：

```bash
bun run .agents/skills/wechat-article-write/scripts/step8-html-convert.mjs \
  <date-slug> [--theme {theme} --color {color}]
```

校验项：`article-wechat.html` 非空、含 inline CSS。

### 8.3 手动调用（仅在脚本不可用时）

如果 `step8-html-convert.mjs` 不可用，可手动执行以下步骤：
...
# 从 article-local.md（本地 imgs/ 路径）生成 HTML
bun run "$HTML_SCRIPT/scripts/main.ts" posts/{date-slug}/article-local.md \
  --theme {theme} \
  --title "{title}"

# 重命名为微信轨专用产物
mv posts/{date-slug}/article-local.html posts/{date-slug}/article-wechat.html
```

**输出路径规则**（重要）：

- `baoyu-markdown-to-html/scripts/main.ts` 的输出路径**固定**为 `{input}.html`，即把输入 md 的扩展名替换为 `.html`
- 输入 `article-local.md` → 输出 `article-local.html`，需要 `mv` 为 `article-wechat.html`
- Step 8 完成后，微信轨一律读 `posts/{date-slug}/article-wechat.html`

**为什么用 article-local.md 而非 article.md**：微信 API 的 `media/uploadimg` 接受图片二进制上传，不需要图片有公网 URL。`article-local.md` 中的 `imgs/NN-xxx.png` 路径被 `wechat-api.ts` 基于相对路径 resolve 为本地文件，直接读取上传——省去"本地→GitHub→CDN→微信API下载→微信CDN"的四跳迂回路径。

### 8.4 完成报告

```
HTML 转换完成！
输出：posts/{date-slug}/article-wechat.html
主题：{theme}
```
...
**执行顺序**：先于 Step 10 微信发布。本步骤将文章同步到博客项目并触发 GitHub Pages 部署。

**前置门控**：进入 Step 9 之前，Step 6（去 AI 痕迹）→ Step 7（格式化）必须完成。Step 8（HTML 转换）不是 Step 9 的前置——博客轨不消费 HTML，Astro Starlight 直接消费 `article.md`（Markdown + CDN URL）。但 Step 8 **是 Step 10 的前置**——微信轨必须消费 `article-wechat.html`。推荐执行顺序：Step 8 → Step 9 → Step 10（或 Step 8 与 Step 9 并行，但两者都必须在 Step 10 之前完成）。

> 自 Task 2 起，Step 4.6 已经把所有插图上传到图床并生成 `image-map.json`，Step 5 已经产出 `article.md`（CDN 版）。Step 9 不再重复上传图片，只做"frontmatter 转换 + 复制到 Starlight + 构建验证 + git push"。**博客轨全程不涉及 HTML 文件的生成或消费。**
...
2. 确定博客文件名：优先使用 `--blog-slug`（纯 ASCII）；未传则从 date-slug 去掉日期前缀，若非纯 ASCII 则报错退出 → 写入 `src/content/docs/articles/{slug}.md`
...
5. `git push` 失败时：状态写 `blocked`，生成 `RESUME.md`（含续跑命令），进程 `exit 0`
6. 写回 `posts/{date-slug}/.pipeline-state.json`：`step=9`, `status=done|blocked`, 记录 commit SHA / blog slug
...
## Step 9.5: 等待 Pages 部署就绪（可选）

**目的**：确保 `https://ntlx.github.io/articles/{slug}/` 真正可访问。**此步骤现在默认跳过**——Step 9 push 完成后直接进入 Step 10。sourceUrl 在 Step 2 创作时已预先填入，publish-wechat.mjs 的探活检查默认关闭（`--skip-deploy-check` 默认为 true）。

**何时手动运行**：如果希望确保"原文链接"零 404，可在 Step 9 之后、Step 10 之前手动执行：

**脚本内置**：`wait-pages-deployed.mjs` 在成功/超时时自动写状态（Step 9.5 done/skipped）。

```bash
bun run .agents/skills/wechat-article-write/scripts/wait-pages-deployed.mjs \
  --slug {slug} --date-slug {date-slug} --timeout 180
```

脚本行为：

- 轮询 `https://ntlx.github.io/articles/{slug}/`，HEAD 失败回退 GET，10s 间隔，默认 180s 超时
- 200 → `exit 0`，脚本内置自动写 `step=9.5 done`（需传入 `--date-slug <date-slug>`，默认与 slug 同值）
- 超时 → `exit 2`，脚本内置自动写 `step=9.5 skipped`；向用户告警并询问是否继续 Step 10（用户可选择稍后人工重发草稿）

> **注意**：调用时传入 `--date-slug` 参数（可选，默认等于 slug），脚本据此定位 `.pipeline-state.json` 路径。


## Step 10: 发布到公众号草稿（后发布）⛔ BLOCKING

**执行顺序**：必须在 Step 8（HTML 转换）和 Step 9（博客发布轨）之后。`sourceUrl`（`https://ntlx.github.io/articles/{slug}/`）在 Step 2 创作时已预先填入 draft.md frontmatter，无需等待 Pages 部署即可发布。

**前置门控**：Step 8（HTML 转换）必须已完成——`article-wechat.html` 是本步骤的必需输入。Step 9 也必须已完成。

使用 `baoyu-post-to-wechat` 技能将文章发布到微信公众号草稿箱。本步骤已脚本化为 `publish-wechat.mjs`，使用 `article-wechat.html`（本地路径版 HTML）+ 本地 `imgs/` 图片——微信 API 直接读本地文件上传，零 CDN 依赖。

**微信轨图片上传路径**：`imgs/` 本地文件 → `wechat-api.ts` `loadUploadAsset()` 通过 `fs.readFileSync` 读取 → `uploadToWechat()` 将 Buffer POST 到微信 API → 图片落在 `mmbiz.qpic.cn`。全程两跳，无需经过 CDN。
...
1. **Pre-flight**：校验 `cover.{png,jpg}` 存在、`article-wechat.html` 存在、`article.md` 含 `sourceUrl`。sourceUrl 探活默认关闭（`--skip-deploy-check` 默认 true）——如果需要验证 URL 可达，显式传 `--no-skip-deploy-check`。探活失败立即 `exit 2`，禁止发布。
2. 直接调用 `baoyu-post-to-wechat` 的 `post-draft.mjs`，传入 `--html article-wechat.html`：
   - `wechat-api.ts` 的 `loadUploadAsset()` 基于 HTML 文件所在目录（`posts/{date-slug}/`）resolve 相对路径 `imgs/NN-xxx.png`，直接 `fs.readFileSync` 读取本地图片上传
   - 全程本地文件操作，零 CDN 依赖，不存在超时降级场景
...
HTML：article-wechat.html（本地路径版，零 CDN 依赖）
```
...
产出物：
  ...
  ✓ 微信公众号 HTML：posts/{date-slug}/article-wechat.html
  ✓ 封面图：posts/{date-slug}/cover.{png|jpg}
...
**已解决**。双轨分离后，博客轨不等待 CDN 传播（Astro 构建不 fetch 图片，用户访问时 CDN 已就绪），微信轨全程使用本地文件（零 CDN 依赖）。jsDelivr 偶发故障仅影响已发布博客文章的图片加载（极少发生且浏览器自动重试），不再影响管线任何环节的执行。
...
```bash
sed -i '' -e 's|a|b|g' -e 's|c|d|g' article-wechat.html
```
...
### Step 8 被跳过导致 Step 10 失败

**现象**：Agent 在执行流水线时跳过 Step 8（HTML 转换），直接执行 Step 9（博客发布）和 Step 10（微信草稿）。Step 10 的 pre-flight 校验因 `article-wechat.html` 不存在而失败。

**原因**：Step 8 之前缺少 ⛔ BLOCKING 标记和"禁止跳过"警告，且 Step 9 的前置门控描述明确说"Step 8 不是 Step 9 的前置"——Agent 误以为 Step 8 可以跳过或延后。虽然对博客轨而言 Step 8 确实不必要，但 Step 10（微信轨）强依赖 `article-wechat.html`。

**解决**：Step 8 现已标记 ⛔ BLOCKING + "⚠️ 禁止跳过"。Step 9 前置门控描述已补充"Step 8 是 Step 10 的前置"。Step 10 前置门控已明确声明"Step 8 必须已完成"。
