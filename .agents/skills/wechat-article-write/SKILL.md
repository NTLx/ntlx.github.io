...
1. 读取 `.baoyu-skills/baoyu-imagine/EXTEND.md`，确认 `preferred_image_backend: dashscope`；如果不是 DashScope，先修正配置或停止报告
...
6. 运行 `step4-generate.mjs` 执行批量生成（只使用 DashScope；失败只重试 DashScope，不做后端降级）
...
```
脚本读取 `imgs/batch.json` → 跳过已有图片 → 强制把待生成任务 provider 设为 DashScope → 调用 baoyu-imagine batch → 失败项逐个用 DashScope 重试。退出码：0 全部成功，4 部分失败。
