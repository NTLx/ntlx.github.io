...
**扩展模式传参**：在调用 ljg-writes 的 Skill 参数中追加以下指令：
```
扩展模式：材料丰富，目标 2000-3000 字。允许分章节深入展开多个论点。每个论点用「切」方法独立深挖，但共享同一个核心观点。覆盖 ljg-writes 的 1000-1500 字约束和「一篇只说一个点」限制——不是多个点浅层罗列，而是多个论点围绕同一核心各自切到底。
```
...
HTML 转换脚本从 jsDelivr CDN 下载文章插图时，**新上传的图片几乎必定超时**——CDN 需要 2-3 分钟传播，而 Step 5 到 Step 9 的间隔通常只有几十秒。不用浪费重试次数。

**策略（缩减版）**：
1. 首次尝试 CDN 直连（15s 预防性等待后）
2. 超时 → 等待 30s，重试 1 次
3. 仍超时 → **直接走本地路径降级**（不再重试第 3 次）

**为什么只重试 1 次**：CDN 传播延迟 2-3 分钟是系统性问题，3 次重试（20s+30s+45s+执行时间≈2 分钟）叠加后仍可能在传播窗口之外。重试 1 次确认不是瞬时抖动后，本地路径降级是更快更可靠的路径。
...
### 11.1 上传封面图到 GitHub 图床

将封面图片上传到 GitHub 图床，获取 CDN URL：

```bash
bun run .agents/skills/github-image-hosting/scripts/upload.ts \
  --file "posts/{date-slug}/cover.png" \
  --name "wechat-articles/{slug}-cover.png"
```

如果封面是 `.jpg` 格式，替换文件扩展名即可。上传失败时重试 1 次，仍失败则跳过封面图（标记为 SKIPPED）。

记录返回的 CDN URL：`{cover_cdn_url}`
...
# 移除 coverImage 字段（封面图已作为正文首行）
content = re.sub(r'^coverImage:.*\n', '', content, flags=re.MULTILINE)
# 在 frontmatter 结束后、正文开始前插入封面图
if '{cover_cdn_url}':
    content = content.replace('---\n', '---\n', 1) + ''  # frontmatter 已闭合
    # 在第一个 --- 之后的 --- 后面插入封面图
    parts = content.split('---\n', 2)
