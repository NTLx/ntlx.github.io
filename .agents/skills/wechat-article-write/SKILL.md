...
- **插图三匹配铁律（位置·数量·内容）**：每个 H2 章节必须有且仅有一张插图，同时满足三个条件：①**位置**——`<!-- SLOT_IMG_XX -->` 占位符紧跟在 `## ` 标题之后、章节正文之前（`## 标题` → `<!-- SLOT_IMG -->` → 段落文字）；②**数量**——每个内容章节至少一张，禁止遗漏；③**内容**——插图的 prompt 必须从**所在章节的实际内容**提取关键概念、数据或关系来构建，是该章节的视觉摘要，禁止用笼统/通用描述代替。Step 4 的 `generate-image-prompts.mjs` 必须逐章节读取正文后再生成对应 prompt。**禁止将插图放在段落之间、章节末尾或任何非标题直下方的位置**。step4-images.mjs 校验位置约束，不符合则 fail
...
- [ ] **每个文内插图 prompt 必须从对应章节正文提取关键概念/数据/关系**，是该章节的视觉摘要；禁止用笼统描述（如"a diagram about AI"）替代章节具体内容
...
脚本执行：缺 draft.md/cover 直接 fail → 统一格式检测修正 → SLOT-only 强制校验 → SLOT_IMG 与 imgs/ 一一对应 → 每个 H2 章节必须有 SLOT_IMG → **SLOT_IMG 必须紧跟 H2 标题之后（位置校验）** → 信息图位置约束。全部通过写状态。
