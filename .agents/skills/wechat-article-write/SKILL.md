...
| 8 | HTML 转换（微信轨专用） | baoyu-markdown-to-html（从 article-local.md 生成） | article-wechat.html | |
...
**脚本内置**：使用 `step8-html-convert.mjs` 一键完成 HTML 转换 + 重命名 + 验证 + 状态写入。
...
**前置门控**：进入 Step 9 之前，Step 6（去 AI 痕迹）→ Step 7（格式化）必须完成。Step 8（HTML 转换）不是 Step 9 的前置——博客轨不消费 HTML，Astro Starlight 直接消费 `article.md`（Markdown + CDN URL）。
...
**执行顺序**：必须在 Step 9（博客发布轨）之后。`sourceUrl`（`https://ntlx.github.io/articles/{slug}/`）在 Step 2 创作时已预先填入 draft.md frontmatter，无需等待 Pages 部署即可发布。
