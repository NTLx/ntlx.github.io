...
当 baoyu-imagine batch 或 baoyu-article-illustrator 批量生成结果中**部分图片成功、部分失败**时（常见原因：DashScope 内容审核拒绝复杂 prompt），遵循以下策略：

1. **不要重跑整个 batch**——已成功生成的图片无需重复，重跑浪费时间且可能因并发限制导致原本成功的也失败
2. **仅对失败的图片逐个重试**，使用简化后的 prompt：
   - 移除可能触发审核的敏感词汇（暴力、政治、医疗等）
   - 用更抽象/隐喻的描述替代具象描述（如 "安全攻防" → "红蓝对抗棋盘"）
   - 缩短 prompt 长度，保留核心视觉元素
3. 重试时用 baoyu-imagine **单图模式**（`--prompt` + `--image`），不走 batch
4. 如果简化 prompt 仍失败，**降级后端**（Gemini → Seedream → DashScope），而非继续优化 prompt
5. 单张图片重试 2 次仍失败，跳过该图并记录，不要卡住整个管线——Step 4 脚本会检查 SLOT 与文件映射，缺失图片可后续补生成

**baoyu-imagine batch 字段名**：batch JSON 中每个 task 的 prompt 文件字段名为 `promptFiles`（数组，驼峰 F），不是 `promptfile`。示例：
```json
{
  "id": "illustration-01",
  "promptFiles": ["prompts/01-scene-concept.md"],
  "image": "imgs/01-scene-concept.png"
}
```
...
3. **批量图片部分失败**：仅对失败图片逐个重试（简化 prompt），已成功的不要重新生成。详见 Step 4 "批量图片部分失败时的增量重试"段落
4. 图床上传失败：报告错误并保留状态；修复网络或凭据后重跑 Step 5；若 `image-map.json` 已完整生成，可用 `--reuse-image-map` 避免重复上传
5. 其他失败：保存中间产物和状态，报告进度
6. `state.mjs next <date-slug>` 返回下一个待执行步骤，全部完成时返回 `done`
7. Step 6 发布分为博客和微信两个子状态：`state.mjs blog <slug> get` 和 `state.mjs wechat <slug> get` 可独立查询；合法组合包括：blog=done+wechat=done（全部完成）、blog=blocked+wechat=done（push 失败但微信已发）、blog=done+wechat=failed（需重试微信）
...
| `scripts/state.mjs` | 状态 CLI（init/get/next/done/fail/blog/wechat/dump；blog/wechat 支持 get 查询子状态） |
