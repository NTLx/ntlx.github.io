...
| **10** | **发布到公众号草稿**（后发布） | publish-wechat.mjs（sourceUrl 已预先填入） | 公众号草稿 | ⛔ |
...
**脚本内置**：使用 `step46-upload-cdn.mjs` 一键完成上传 + CDN 等待 + 状态写入。
...
- `baoyu-markdown-to-html/scripts/main.ts` 的输出路径**固定**为 `{input}.html`，即把输入 md 的扩展名替换为 `.html`，写到**同目录**。**不接受 `--output` 参数**（传了会被忽略）。
- CDN 模式：输入 `posts/{date-slug}/article.md` → 输出 `posts/{date-slug}/article.html`
- 本地降级模式：输入 `posts/{date-slug}/article-local.md` → 输出 `posts/{date-slug}/article-local.html`，**`run-with-cdn-fallback.sh` 会自动 `mv` 为 `article.html`**，保持下游 Step 9/10 看到的产物名一致
- Step 8 完成后，下游一律读 `posts/{date-slug}/article.html`，不要去找 `article-local.html` 或 `article-formatted.html`
...
- `--stage html` 时，本地降级成功后会先把 `article-local.html` 重命名为 `article.html`，再调用 `apply-image-map.mjs --html-rewrite` 把生成的 `article.html` 内的本地路径回写为 CDN URL
- 占位符 `{ARTICLE_MD}` 由脚本按当前模式自动替换为 `article.md` 或 `article-local.md`

### 8.3 验证与状态

**脚本内置**：使用 `step8-html-convert.mjs` 一键完成 HTML 转换 + 验证 + 状态写入。
...
1. **Pre-flight**：校验 `cover.{png,jpg}` 存在、`article.md` 含 `sourceUrl`。sourceUrl 探活默认关闭（`--skip-deploy-check` 默认 true）——如果需要验证 URL 可达，显式传 `--no-skip-deploy-check`。探活失败立即 `exit 2`，禁止发布。
