...
6. Step 6 发布分为博客和微信两个子状态：`state.mjs blog <slug> get` 和 `state.mjs wechat <slug> get` 可独立查询；合法组合包括：blog=done+wechat=done（全部完成）、blog=blocked+wechat=done（push 失败但微信已发）、blog=done+wechat=failed（需重试微信）
