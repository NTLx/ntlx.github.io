...
- **renwei-writing 强制执行**：Step 3 必须通过 Skill 工具调用 `renwei-writing` 技能处理正文。这是消除 AI 写作痕迹的唯一防线，跳过 = 发布带 AI 味的文章。**禁止以"文章质量已足够""时间不够""内容无需调整"等任何理由跳过**。tutorial 策略（已标记 `humanizer: skip`）是唯一例外
- **插图三匹配铁律（位置·数量·内容）**：文内插图由 Agent 根据文章内容判断放在最能解释观点的位置，必须同时满足三个条件：①**位置**——`<!-- SLOT_IMG_XX -->` 占位符靠近其解释的概念、流程、对比、数据或关系，可位于 H2 后、关键段落后或小结前，但不要为凑数机械塞图；②**数量**——每篇文章至少 3 张文内插图，且不包含封面图和 SLOT 00 头部信息图；③**内容**——插图 prompt 必须从占位符附近正文提取关键概念、数据或关系来构建，是该处论证的视觉摘要，禁止用笼统/通用描述代替。写作时尽量先规划好 SLOT_IMG_01+ 的位置和语义描述，Step 4 的 `generate-image-prompts.mjs` 基于占位符上下文生成对应 prompt。step2/3/4 会校验文内插图总数不少于 3 张
...
**角色分工**：Agent 负责运行脚本生成符合规范的 prompt 文件；脚本会为 SLOT 00 信息图引用 `gpt-image-2` 的紧凑模板风格，为封面和文内插图继续使用 baoyu 模板。然后在主会话中用 Bash 串行调用 baoyu-image-gen 逐张生成图片。**Agent 不得绕过模板体系手写 prompt**。

### 为什么必须用技能模板

`gpt-image-2` 和 baoyu 系列技能的 prompt 模板包含针对特定文生图模型的渲染指令：

| 模板元素 | 所在文件 | 缺失后果 |
|----------|---------|---------|
| `hand-drawn wobble` / `bullet journal infographic` | gpt-image-2 infographic template | 信息图风格漂移、像普通 PPT |
...
读取 EXTEND.md 确认 provider
  → 运行 generate-image-prompts.mjs 生成规范 prompt 文件
...
脚本会生成封面、SLOT 00 信息图和 SLOT 01+ 文内插图 prompt。SLOT 00 使用 `gpt-image-2/references/infographics/*.md` 的模板风格，但只输出紧凑字段 prompt（包含 `Template source`、use case、subject、style、composition、constraints），避免把完整模板全文塞进文生图 prompt；封面和文内插图继续按 Skill 查找优先级解析 baoyu 模板目录（项目级 `.agents/skills/` 优先）。若 prompt 文件已存在，脚本默认跳过；需要重新生成时显式传 `--overwrite`。

Agent 必须审核脚本输出是否满足下方质量检查清单。若脚本生成的文章语义不够精准，只允许在生成出的 prompt 文件上按对应模板补充内容，不要绕过模板手写一份全新的 prompt。

### 手工 fallback：按图片类型走对应的 prompt 构建流程

#### 信息图（SLOT_IMG_00）：使用 gpt-image-2 紧凑模板引用

1. 按文章类型和 `image-plan.json` 解析 `infographic.layout` / `infographic.style`
2. 映射到 `gpt-image-2/references/infographics/` 下的具体模板风格：
   - `morandi-journal` / `craft-handmade` / `aged-academia` → `hand-drawn-infographic.md`
   - `linear-progression` / `circular-flow` / `winding-roadmap` → `step-by-step-infographic.md`
   - `comparison-matrix` / `binary-comparison` / `venn-diagram` → `comparison-infographic.md`
   - `dashboard` → `kpi-dashboard-infographic.md`
   - 其它总览类布局 → `bento-grid-infographic.md`

3. 输出短字段 prompt，而不是展开完整模板文件。prompt 必须包含：`Template source`、`Use case: infographic-diagram`、`Subject`、`Style/medium`、`Composition/framing`、`Text (verbatim)`、`Constraints`。可见文字优先中文；仅保留模型名、产品名、API、代码标识、英文缩写等不适合翻译的专有名词

4. 保存到 `imgs/prompts/00-infographic-core-summary.md`

#### 文内插图（SLOT_IMG_01+）：使用 baoyu-article-illustrator 技能模板
...
- [ ] 信息图 prompt 使用 `gpt-image-2` 紧凑模板引用，包含 `Template source`，且不展开完整 `Layout Guidelines` / `Style Guidelines`
- [ ] **每个文内插图 prompt 必须从占位符附近正文提取关键概念/数据/关系**，是该处论证的视觉摘要；禁止用笼统描述（如"a diagram about AI"）替代具体内容
...
脚本执行：缺 draft.md/cover 直接 fail → 统一格式检测修正 → SLOT-only 强制校验 → SLOT_IMG 与 imgs/ 一一对应 → 文内插图总数不少于 3 张（不含封面和 SLOT 00 信息图）→ 信息图位置约束。全部通过写状态。
...
step3 复用 step2 的大部分校验逻辑（frontmatter 完整性、H1 检测、SLOT_IMG_00 信息图和至少 3 张文内插图占位符、互动问题、原文参考），错误信息格式相同。如果 step3 报了和 step2 一样的错，说明 renwei-writing/format-markdown 后处理过程中丢失了某些内容。
