...
> **所有 Step 出口处必须写 `.pipeline-state.json`**：`bun run scripts/state.mjs set <date-slug> <step> done`。断点续跑用 `bun run scripts/state.mjs next <date-slug>` 输出第一个非 done 的 Step 编号。**状态写入不是可选的**——每个 Step 完成后必须立即写入，否则 `state.mjs next` 无法准确定位续跑起点。
...
bun run .agents/skills/wechat-article-write/scripts/state.mjs \
  set <date-slug> 1 done '{"sources":<成功数>,"failed":<失败数>}'
```
...
```bash
bun run .agents/skills/wechat-article-write/scripts/state.mjs \
  set <date-slug> 2.5 done '{"category":"<chosen>","blogSlug":"<ascii-slug>"}'
```
...
```bash
bun run .agents/skills/wechat-article-write/scripts/state.mjs \
  set <date-slug> 3 done '{"cover":"cover.{png|jpg}"}'
```
...
```bash
bun run .agents/skills/wechat-article-write/scripts/state.mjs \
  set <date-slug> 4 done '{"images":<插图数>,"refs_ok":true}'
```
...
```bash
bun run .agents/skills/wechat-article-write/scripts/state.mjs \
  set <date-slug> 4.5 done '{"infographic":"00-infographic-core-summary.{ext}"}'
```
