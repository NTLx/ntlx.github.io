...
function makeState(postsRoot, slug, lastCompleteStep, strategy) {
...
describe("pipeline", () => {
  ...
  test("illustrate contract keeps Codex runtime preflight explicit", () => {
    const root = join(tmpdir(), `pipeline-illustrate-test-${Date.now()}-${Math.random().toString(36).slice(2)}`);
    const postsRoot = join(root, "posts");
    cleanup.push(root);
    const slug = "2026-05-17-pipeline-illustrate";
    makeState(postsRoot, slug, 3, "reader-response");
